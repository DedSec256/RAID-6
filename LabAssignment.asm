...
.DATA 
		...
		HIGH_BITS_MASK QWORD 8080808080808080h  ; Маска выделения старшего бита каждого байта в 8-байтном блоке
...
INNER_LOOP:
		...
		; Получаем маску, которая прибавляет множимое к сумме
		; Только если у множителя текущий старший бит не ноль
...
INNER_LOOP: 
		...
		; Умножаем полученное значение Q на x
		CALL MUL_X
...
Recover PROC
		...
		; в RAX		будет лежать адрес Q_sum (т.е. и адрес Q = N*32 + 32)
